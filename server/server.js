...
app.post("/pokedex", async (req, res) => {
  try {
    ...
    const pokemon = parameters.pokemon
      ? parameters.pokemon
          .toLowerCase()
          .replace(".", "-")
          .replace(" ", "")
    ...
    if (
      pokemon_species_endpoint.indexOf(specs) !== -1 ||
      ...
      if (intent.displayName == "evolution") {
        const evolution_response = await api.get(
        ...
        if (evolution_response.data.chain.evolves_to[0].evolves_to.length) {
          pokemon_evolutions.push(
            evolution_response.data.chain.evolves_to[0].evolves_to[0].species
              .name
          );
        }
    ...
    if (get_type_effectiveness) {
      ...
      const type_effectiveness_formatted = type_effectiveness.replace(
        /_/g,
        " "
      );
      const type_effectiveness_word =
        outputContexts[0].parameters["type_effectiveness.original"];

      let from_or_to = type_effectiveness.split("_")[2];

      const pokemon_type_comes_first =
        queryText.indexOf(pokemon_type) <
        queryText.indexOf(type_effectiveness_word)
          ? true
          : false;
      ...
      const response = await api.get(`/type/${pokemon_type}`);
      const damage_relations =
        response.data.damage_relations[type_effectiveness].length > 0
          ? response.data.damage_relations[type_effectiveness]
              .map(item => item.name)
              .join(", ")
          : "none";
